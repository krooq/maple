...
impl Pipeline {
    pub fn new(
        ...
        let constant_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            bindings: &[wgpu::Binding {
                binding: 0,
                resource: wgpu::BindingResource::Buffer(wgpu::BufferSlice {
                    buffer: &constants_buffer.raw,
                    offset: 0,
                    size: wgpu::BufferSize(std::mem::size_of::<Uniforms>() as u64),
                }),
            }],
        });
        ...
        let vs = include_bytes!("shader/iced/triangle.vert.spv");
        let vs_module = device.create_shader_module(
            &wgpu::read_spirv(std::io::Cursor::new(&vs[..]))
                .expect("Read triangle vertex shader as SPIR-V"),
        );

        let fs = include_bytes!("shader/iced/triangle.frag.spv");
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            sample_count: u32::from(antialiasing.map(|a| a.sample_count()).unwrap_or(1)),
            sample_mask: !0,
    ...
    pub fn draw(
        ...
        for mesh in meshes {
            let transform =
                (transformation * Transformation::translate(mesh.origin.x, mesh.origin.y)).into();
            ...
            encoder.copy_buffer_to_buffer(
                ...
                (std::mem::size_of::<Vertex2D>() * mesh.buffers.vertices.len()) as u64,
            );
            ...
            encoder.copy_buffer_to_buffer(
                ...
                (std::mem::size_of::<u32>() * mesh.buffers.indices.len()) as u64,
            );
        ...
        {
            let (attachment, resolve_target, load_op) = if let Some(blit) = &mut self.blit {
            ...
            for (i, (vertex_offset, index_offset, indices)) in offsets.into_iter().enumerate() {
                ...
                render_pass.set_index_buffer(wgpu::BufferSlice {
                    buffer: &self.index_buffer.raw,
                    offset: 0,
                    size: wgpu::BufferSize(index_offset * std::mem::size_of::<u32>() as u64),
                });
